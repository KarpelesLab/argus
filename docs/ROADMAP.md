...
| 3 — Chrome, navigation & services | 🟡 links → fetch → re-render, URL + subresource resolution, **scroll-wheel**, **persistent cookie jar**, **conservative HTTP cache** (Cache-Control-aware). Tabs/history/back-forward, CSP, cache revalidation (ETag/Expires) remain |
