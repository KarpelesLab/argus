...
| 7 — Hardening / perf / conformance | 🟡 started — parser + **full layout-pipeline** robustness tests (random inputs) + cargo-fuzz harness (html/css/**layout**) + **image-decoder fuzz** (all formats fail closed on hostile bytes) + **DOM-ops JSON fuzz**, accessibility tree. WPT, perf, sandbox hardening remain |
